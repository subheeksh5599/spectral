...
| G1: a taker who never finishes forfeits the bond to the buyer, by rule | **Real — executed on the deployed contract** | `docs/LIVE-GATES.md` job 3: buyer credit 0.00375 = remainder 0.0025 + bond 0.00125, executor credit 0.0005, taker 0; credits sum == venue balance, read back from the chain |
| Seven invalid actions refused by the deployed bytecode | **Real — evaluated by the live contract** | `docs/LIVE-GATES.md`: `UnitAlreadyCounted`, `EmptyReceipt`, `UnitOutOfRange`, `NotExecutor`, `NothingToTake`, `AlreadyStalled`, arithmetic panic — each with the contract's own reason, raw output in `docs/live-gates-raw.json` |
| Reentrancy on the payout path | **Real — attacked and refused** | `testReentrantClaimCannotDrainAnotherJob` constructs the attacker, re-enters from the receive callback, and proves a second job's money is untouched |
| Zero price and overflowing escrow | **Real — tested, and one mined on chain** | `testZeroPriceJobIsVacuousNotUnsafe` (escrows nothing, settles to nothing), `testCreateRejectsOverflowingEscrow`, and the live overflow attempt in `docs/LIVE-GATES.md` |
...
| Participation by anyone outside this build | **Not yet — stated plainly** | no wallet other than the three testnet actors has taken over an obligation, and no third party has described the mechanism in public. Both are listed as open in `CHECKLIST.md` (H1, H2) rather than implied: the mechanism is exercised, the market for it is not. |
| Faucet and explorer reachability | **Real** | the faucet link sits in the Open-a-job view where funding is needed; every board row's executor address and the detail panel's contract row open the explorer |
...
| Layered scroll on the metric deck | **Restored — verified in a scroll trace** | cards pin at their own offsets (112 / 144 / 176px) with z-index 10/20/30, and the tuck choreography fires on the ones underneath: measured `scale(0.985) translateY(-2.9px)` on the first card as the second arrives, deepening to `scale(0.96) translateY(-8px)` with opacity 0.85. It had been silently disabled by an `overflow-x: hidden` on the wrapper element; that now lives on `<body>` only, where it does not create a scroll box |
| No live chain data on the landing | **True by construction, checked** | the page contains no address, no chain name, no faucet link and no per-job figures; the only numbers are the contract's design facts (6 states, 4 refusals, 0 reporters), shown as the design specifies |
...
| Hosted public URL | **Real — verified anonymously** | https://spectral-venue.vercel.app — `/` and `/app` return 200 with no login, `/api/config` serves the live chain values, and the venue reads four jobs off the contract in a browser with no wallet installed |
