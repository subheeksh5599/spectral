...
| The deployed bytecode is the source in this repository | **Real — 4/4 rebuilt and matched** | Sourcify, free and keyless on chain 1952: `Spectral` `match` (ID 52236721), `SpectralToken` `exact_match` (ID 52236800), `TestnetEquity` `exact_match` (ID 52236822), `SpectralToken (USD)` `exact_match` (ID 52289156). Re-checked by `verify_source.py`; levels and commands in [docs/SOURCE-VERIFICATION.md](docs/SOURCE-VERIFICATION.md) |
...
| A takeover by someone outside this build | **Not yet — but four are listed and takeable now, by anyone** | native job #6 (0.005 OKB bond), token job #3 (4.5 tTSLA bond) and two in the dollar market (2.000000 USD₮0 bond each, one with a day-long taker window) are live listings any wallet can take today, **from the page** — each market's panel carries the take button, and the terminal route is in [Take a live obligation yourself](#take-a-live-obligation-yourself) for anyone who prefers it. The listings were announced from the project's own account ([the post](https://x.com/KomariS18774/status/2103385501424308286)), which is a call for a taker rather than a taker. The five takeovers that *have* been signed from the page came from wallets this build generated for the test, holding no key in this repository, which is the closest this project can honestly get to an outside taker on its own |
| Distribution of the reader | **In the repo, not on the registry** | `npm/spectral-board/` packs to 6 files / 6.4 kB, installs clean from its own tarball into an empty project and runs every mode against the live chain; it is not published and nothing here asks a reader to `npx` it. The path that needs no install is `GET /api/board`, keyless |
...
The same reader is also packaged under `npm/spectral-board/` (6 files, 6.4 kB) for anyone who wants
it as a dependency, but this README does not ask you to run it: the no-install path is the keyless
`GET /api/board` above, and the package is not on the registry.
